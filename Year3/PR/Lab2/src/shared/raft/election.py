    ...
    def _run_election_loop(self):
        ...
        while True:
            ...
            if self.state.state == ServerState.LEADER:
                ...
                print(time.time() - self.state.last_heartbeat, self.state.election_timeout, self.state.state)
                if time.time() - self.state.last_heartbeat > self.state.election_timeout:
                    ...
                    print(self.state.state, self.state.current_term, self.state.server_id, self.state.voted_for)
                time.sleep(5)
